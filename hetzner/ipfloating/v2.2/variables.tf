...
variable "home_location" {
  type        = string
  description = "Home location for floating IPs (ignored if server_ids provided)"
  default     = null
}
...
variable "server_ids" {
  type        = list(number)
  description = "Optional list of server IDs to assign FIPs to. If set, home_location will be ignored."
  default     = []
}